...
const LoginPage: React.FC = () => {
  ...
  const { login, register, loading, debugLogin } = useAuth();
  const { t } = useLanguage();
      ...
      <div className="w-full max-w-md p-10 space-y-8 bg-card-dark rounded-[3rem] border border-white/10 shadow-2xl animate-scaleIn relative z-10">
        ...
        <form className="space-y-5" onSubmit={handleSubmit}>
          ...
          <button
            type="button"
            onClick={debugLogin}
            className="w-full py-3 rounded-2xl border border-white/10 hover:bg-white/5 text-[10px] font-black uppercase tracking-widest text-white/40 hover:text-white transition-all"
          >
            Skip Login (Debug)
          </button>
        </form>
